...
describe('ReadMore', () => {
  it('should mount without throwing an error', () => {
    const wrapper = mountEnzymeWithTheme(<ReadMore expandedText={<h1>{text}</h1>}>{text}</ReadMore>);
    expect(wrapper).toHaveLength(1);
  });

  it('should render collapsed text when collapsed', () => {
    const rendered = renderEnzymeWithTheme(<ReadMore expandedText={<h1>{text}</h1>}>{text}</ReadMore>);
