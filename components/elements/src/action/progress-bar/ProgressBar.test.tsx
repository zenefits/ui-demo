...
describe('ProgressBar', () => {
  it('should mount without throwing an error', () => {
    const wrapper = mountEnzymeWithTheme(<ProgressBar />);
    expect(wrapper.find('progress')).toHaveLength(1);
  });

  it('should support standard button attributes', () => {
    const rendered = renderEnzymeWithTheme(<ProgressBar id="foo" value={50} max={100} />);
  ...
  describe('should support util props', () => {
    it('width prop', () => {
      const rendered = mountEnzymeWithTheme(<ProgressBar value={50} max={100} w={123} />);
      expect(rendered).toHaveStyleRule('width', '123px');
    });

    it('color prop', () => {
      const rendered = mountEnzymeWithTheme(<ProgressBar value={50} max={100} color="secondary.a" />);
      expect(rendered).toHaveStyleRule('color', getColor('secondary.a'));
    });

    it('bg prop', () => {
      const rendered = mountEnzymeWithTheme(<ProgressBar value={50} max={100} bg="grayscale.g" />);
  ...
  it('should invoke callback onChange', () => {
    ...
    const wrapper = mountEnzymeWithTheme(<ProgressBar onChange={onProgressChange} />);
