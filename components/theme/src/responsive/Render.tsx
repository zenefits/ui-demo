...
type Props = {
  /** Render children when in embedded native view.
   * @default false */
  inEmbeddedNativeView?: boolean;

  /** Which breakpoints should children be rendered in?
   * @default []
   * Example: [true] means render only at smallest breakpoint. */
  forBreakpoints?: boolean[];
};
type State = { breakpointMatches: boolean[] };

export default class Render extends React.Component<Props, State> {
