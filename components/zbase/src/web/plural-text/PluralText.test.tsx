...
describe('PluralText', () => {
  it('should render a `<span>` by default', () => {
    const rendered = renderEnzymeWithThemeIntl(<PluralText other="Found some reviews" count={2} />);
    expect(rendered.is('span')).toBe(true);
  });

  it('should render a custom tag', () => {
    const rendered = renderEnzymeWithThemeIntl(<PluralText tag="small" other="Found some reviews" count={2} />);
    expect(rendered.is('small')).toBe(true);
  });

  describe('chooses inline message based on count', () => {
    it('zero count', () => {
      const rendered = renderEnzymeWithThemeIntl(<PluralText none="No reviews" other="Found some reviews" count={0} />);
    ...
    it('interpolated count', () => {
      const rendered = renderEnzymeWithThemeIntl(<PluralText other="{count} reviews" count={2} />);
      expect(rendered.text()).toBe('2 reviews');
    });

    it('supports formatted count', () => {
      const rendered = renderEnzymeWithThemeIntl(<PluralText other="{count, number} reviews" count={12345} />);
  ...
  describe('chooses external message based on count', () => {
    it('zero count', () => {
      const rendered = renderEnzymeWithThemeIntl(<PluralText noneKey="app.review.count.none" count={0} />, {
        messages,
      });
    ...
    it('single count', () => {
      const rendered = renderEnzymeWithThemeIntl(<PluralText oneKey="app.review.count.one" count={1} />, { messages });
      expect(rendered.text()).toBe(messages['app.review.count.one']);
    });

    it('multiple count', () => {
      const rendered = renderEnzymeWithThemeIntl(<PluralText otherKey="app.review.count.other" count={2} />, {
        messages,
      });
    ...
    it('interpolated value', () => {
      const rendered = renderEnzymeWithThemeIntl(
        <PluralText otherKey="app.review.count.interpolated.other" count={2} />,
        {
          messages,
        },
      );
    ...
    it('falls back if no key found', () => {
      const rendered = renderEnzymeWithThemeIntl(
        <PluralText noneKey="does.not.exist" none="none fallback" count={0} />,
        {
          messages,
        },
      );
  ...
  it('should respect util props', () => {
    const mounted = mountEnzymeWithThemeIntl(<PluralText p={123} other="{count} reviews" count={2} />);
  ...
  it('should not render font-style util prop as attributes', () => {
    const rendered = renderEnzymeWithThemeIntl(<PluralText p={123} other="{count} reviews" count={2} />);
