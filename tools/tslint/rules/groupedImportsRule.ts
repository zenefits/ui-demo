...
enum ImportType {
  LIBRARY_IMPORT = 1,
  REGEX_PATH_IMPORT = 2, // matches provided regex
  CURRENT_DIRECTORY_IMPORT = 3, // starts with "./"
}

interface Options {
  groupedImportsRegex: string;
...
class Walker extends Lint.AbstractWalker<Options> {
  private readonly importsBlocks = [new ImportsBlock(this.options.groupedImportsRegex)];
  ...
  private checkSource(source: string, node: ImportDeclaration['node'], originPath: string) {
...
class ImportsBlock {
  ...
  private groupedImportsRegex: RegExp;
  constructor(groupedImportsRegex: string) {
  ...
  private getImportType(sourcePath: string, originPath: string): ImportType {
    let type;
